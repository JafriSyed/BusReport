import { EventEmitter, ChangeDetectorRef, Compiler, ComponentFactoryResolver } from '@angular/core';
import { DomSanitizer, SafeHtml } from '@angular/platform-browser';
export declare class NgTableComponent {
    private sanitizer;
    ref: ChangeDetectorRef;
    private componentFactoryResolver;
    private compiler;
    expandedComponent: any;
    rows: Array<any>;
    expandable: boolean;
    editingRow: number;
    editingColumn: number;
    rowInputs: any;
    config: any;
    tableChanged: EventEmitter<any>;
    cellClicked: EventEmitter<any>;
    expanderClicked: EventEmitter<any>;
    scrolledDown: EventEmitter<any>;
    valueEdit: EventEmitter<any>;
    showFilterRow: Boolean;
    showExpandedRow: Boolean;
    expandedRowIndex: number;
    currentChanges: any;
    columns: Array<any>;
    private _columns;
    private _config;
    scrollPercentage: number;
    constructor(sanitizer: DomSanitizer, ref: ChangeDetectorRef, componentFactoryResolver: ComponentFactoryResolver, compiler: Compiler);
    sanitize(html: string): SafeHtml;
    readonly configColumns: any;
    onChangeTable(column: any): void;
    toggleRowExpansion(row: any, rowNum: number): void;
    checkScroll(event: any): void;
    getData(row: any, propertyName: string): string;
    cellClick(row: any, column: any): void;
    setRowToEdit(rowIndex: any, column: any): void;
    updateChanges(colName: any, val: any): void;
    submitChange(row: any, rIndex: number, columnName: string, cIndex: number, newVal: any): void;
}
